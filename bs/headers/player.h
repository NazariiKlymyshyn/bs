...
class Player {
  ...
  struct BattleField {
    ...
    void SeUnitState(std::array < Unit, 100 > & BattleField,
      const uint_fast16_t & Absciss,
        const uint_fast16_t & Ordinate,
          const Unit & UnitState);
    std::array < Unit, 100 > battle_field = {
      state
    };
   ...
   */
  const uint_fast8_t isShip(const uint_fast16_t size,
   ...
   *fields of the class                                                                                            
   */
  const Player::Unit GetSingleUnitState(const uint_fast16_t & Absciss,
    const uint_fast16_t & Ordinate) const;

  void SetAIFieldState(const std::array < Player::Unit, 100 > & Field);
  bool ShipIsGood(int size, bool is_horiz, int row_top, int col_left,
   ...
   */
  const Player::Unit GetSingleUnitState(const std::array < Player::Unit, 100 > & BattleField,
    const uint_fast16_t & Absciss,
      const uint_fast16_t & Ordinate) const;
   ...
   */
  void ProcessClick(const uint_fast16_t & Absciss,
    const uint_fast16_t & Ordinate);
};
